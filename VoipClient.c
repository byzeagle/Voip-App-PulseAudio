...
int create_UDP_socket_send(struct sockaddr_in * server)
{
    int sock;
...
void* recorder()
{
    ...
    struct sockaddr_in server;
    int socket = create_UDP_socket_send(&server);
    ...
    for (;;) 
    {
    	...
    	if((nsent = sendto(socket, buf, BUFFERSIZE, 0, (struct sockaddr *)&server, sizeof(server))) < 0)
		{
			perror("sendto()");
			pa_simple_free(s);
			exit(EXIT_FAILURE);
...
int main(int argc, char * argv[])
{
    ...
    if( (rc1 = pthread_create(&thread_send, NULL, recorder, NULL)))
    {
